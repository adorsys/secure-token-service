...
public class RemoteKeyRetrieverService implements KeyRetrieverService {
    ...
    public RemoteKeyRetrieverService(
        ...
        this.resourceRetriever = new DefaultResourceRetriever(
                Optional.ofNullable(resourceRetrieverProperties.getHttpConnectTimeout()).orElse(DEFAULT_HTTP_CONNECT_TIMEOUT),
                Optional.ofNullable(resourceRetrieverProperties.getHttpReadTimeout()).orElse(DEFAULT_HTTP_READ_TIMEOUT),
                Optional.ofNullable(resourceRetrieverProperties.getHttpSizeLimit()).orElse(DEFAULT_HTTP_SIZE_LIMIT)
        );
    }
